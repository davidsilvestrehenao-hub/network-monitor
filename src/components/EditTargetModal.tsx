...
interface EditTargetModalProps {
  target: Target;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateTargetData) => void;
}

export function EditTargetModal(props: EditTargetModalProps) {
  ...
  createEffect(() => {
    ...
    setAddress(props.target.address);
  });
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      await props.onSubmit(props.target.id, {
        name: name().trim(),
        ...
        <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full">
          <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-semibold text-gray-900">Edit Target</h3>
              <button
                onClick={handleClose}
                disabled={loading()}
          ...
          <form onSubmit={handleSubmit} class="px-6 py-4">
            <div class="space-y-4">
              ...
              <div>
                <label
                  for="edit-target-address"
                  class="block text-sm font-medium text-gray-700 mb-1"
                >
                  Target Address *
                </label>
                <input
                  id="edit-target-address"
                  type="url"
                  value={address()}
