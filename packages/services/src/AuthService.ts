...
export class AuthService implements IAuthService {
  ...
  private setupEventHandlers(): void {
    this.eventBus.onDynamic(
      ...
      this.handleUserLoginRequested.bind(this) as (
        data?: UserLoginRequestData
      ) => void
    );
    this.eventBus.onDynamic(
      "USER_REGISTER_REQUESTED",
      this.handleUserRegisterRequested.bind(this) as (
        data?: UserRegisterRequestData
      ) => void
    );
    this.eventBus.onDynamic(
      "USER_LOGOUT_REQUESTED",
      this.handleUserLogoutRequested.bind(this) as (
        data?: UserLogoutRequestData
      ) => void
    );
  }
  ...
  async login(credentials: LoginCredentials): Promise<AuthResult> {
    ...
    try {
      ...
      this.eventBus.emitDynamic("USER_LOGGED_IN", {
        email: user.email || "",
        provider: "credentials",
        credentials: { userId: user.id, sessionToken },
      });
  ...
  async register(data: RegisterData): Promise<AuthResult> {
    ...
    try {
      ...
      this.eventBus.emitDynamic("USER_REGISTERED", {
        email: user.email || "",
        name: user.name || undefined,
        provider: "credentials",
        data: { userId: user.id, sessionToken },
      });
  ...
  async logout(sessionToken: string): Promise<void> {
    ...
    try {
      ...
      if (session) {
        ...
        this.eventBus.emitDynamic("USER_LOGGED_OUT", {
  ...
  async refreshSession(sessionToken: string): Promise<AuthResult | null> {
    ...
    try {
      ...
      this.eventBus.emitDynamic("SESSION_REFRESHED", {
        userId: user.id,
  ...
  async updateProfile(userId: string, data: UpdateUserData): Promise<User> {
    ...
    try {
      ...
      this.eventBus.emitDynamic("USER_PROFILE_UPDATED", {
        userId: user.id,
  ...
  async deleteAccount(userId: string): Promise<void> {
    ...
    try {
      ...
      this.eventBus.emitDynamic("USER_ACCOUNT_DELETED", {
        userId,
        deletedAt: new Date(),
      });
