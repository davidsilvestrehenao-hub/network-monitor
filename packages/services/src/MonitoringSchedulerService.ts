...
import type { ISpeedTestService } from "@network-monitor/shared";
import type { IEventBus } from "@network-monitor/shared";
import type { ILogger } from "@network-monitor/shared";
import type { TargetIdData } from "@network-monitor/shared";
import type {
...
export class MonitoringSchedulerService implements IMonitoringScheduler {
  ...
  addTarget(config: MonitoringTargetConfig): void {
    this.logger.debug("MonitoringSchedulerService: Adding target", { config });
    ...
    this.eventBus.emitDynamic("MONITORING_TARGET_ADDED", {
  ...
  removeTarget(targetId: string): void {
    this.logger.debug("MonitoringSchedulerService: Removing target", {
      targetId,
    });
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("MONITORING_TARGET_REMOVED", { targetId });
    }
  }
  ...
  updateTargetConfig(
    ...
    this.logger.debug("MonitoringSchedulerService: Updating target config", {
      targetId,
      config,
    });
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("MONITORING_TARGET_UPDATED", {
        targetId,
  ...
  async start(): Promise<void> {
    this.logger.info("MonitoringSchedulerService: Starting scheduler");

    if (this.running) {
      this.logger.warn(
        "MonitoringSchedulerService: Scheduler is already running"
      );
      return;
    }
    ...
    this.eventBus.emitDynamic("MONITORING_SCHEDULER_STARTED", {});
  }

  async stop(): Promise<void> {
    this.logger.info("MonitoringSchedulerService: Stopping scheduler");

    if (!this.running) {
      this.logger.warn("MonitoringSchedulerService: Scheduler is not running");
      return;
    }
    ...
    this.eventBus.emitDynamic("MONITORING_SCHEDULER_STOPPED", {});
  }

  pause(): void {
    this.logger.info("MonitoringSchedulerService: Pausing scheduler");
    ...
    this.eventBus.emitDynamic("MONITORING_SCHEDULER_PAUSED", {});
  }

  resume(): void {
    this.logger.info("MonitoringSchedulerService: Resuming scheduler");
    ...
    this.eventBus.emitDynamic("MONITORING_SCHEDULER_RESUMED", {});
  }
  ...
  enableTarget(targetId: string): void {
    this.logger.debug("MonitoringSchedulerService: Enabling target", {
      targetId,
    });
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("MONITORING_TARGET_ENABLED", { targetId });
    }
  }

  disableTarget(targetId: string): void {
    this.logger.debug("MonitoringSchedulerService: Disabling target", {
      targetId,
    });
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("MONITORING_TARGET_DISABLED", { targetId });
    }
  }

  pauseTarget(targetId: string): void {
    this.logger.debug("MonitoringSchedulerService: Pausing target", {
      targetId,
    });
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("MONITORING_TARGET_PAUSED", { targetId });
    }
  }

  resumeTarget(targetId: string): void {
    this.logger.debug("MonitoringSchedulerService: Resuming target", {
      targetId,
    });
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("MONITORING_TARGET_RESUMED", { targetId });
    }
  }
  ...
  setDefaultInterval(intervalMs: number): void {
    ...
    this.logger.debug("MonitoringSchedulerService: Default interval set", {
      intervalMs,
    });
  }
  ...
  setMaxConcurrentTests(maxConcurrent: number): void {
    ...
    this.logger.debug("MonitoringSchedulerService: Max concurrent tests set", {
      maxConcurrent,
    });
  }
  ...
  setFailureThreshold(maxFailures: number): void {
    ...
    this.logger.debug("MonitoringSchedulerService: Failure threshold set", {
      maxFailures,
    });
  }
  ...
  private startTargetMonitoring(targetId: string): void {
    ...
    this.logger.debug(
      "MonitoringSchedulerService: Starting target monitoring",
      {
        targetId,
  ...
  private async runTargetTest(targetId: string): Promise<void> {
    ...
    try {
      this.logger.debug("MonitoringSchedulerService: Running test for target", {
        targetId,
      });
      ...
      this.eventBus.emitDynamic("MONITORING_TEST_SUCCESS", {
        targetId,
        duration: Date.now() - startTime,
      });
    } catch (error) {
      this.logger.error("MonitoringSchedulerService: Test failed for target", {
        targetId,
        error,
      });
      ...
      if (target.failureCount >= this.maxFailures) {
        this.logger.warn(
          "MonitoringSchedulerService: Target disabled due to failures",
          {
            targetId,
      ...
      this.eventBus.emitDynamic(EventKeys.MONITORING_TEST_FAILURE, {
        targetId,
  ...
  private handleTargetRemoveRequested(data: TargetIdData): void {
