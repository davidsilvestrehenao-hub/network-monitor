  ...
  describe("Event Handlers", () => {
    ...
    describe("handleUserLogoutRequested", () => {
      it("should handle logout request event", async () => {
        ...
        // Call the handler directly since the mock EventBus doesn't trigger handlers
        await (authService as any).handleUserLogoutRequested(eventData);

        // Should not throw error even for non-existent session
        // No event should be emitted for non-existent session
        expect(mockEventBus.emitDynamic).not.toHaveBeenCalledWith(
          "USER_LOGGED_OUT",
          expect.anything()
        );
