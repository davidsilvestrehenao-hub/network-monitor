...
export class SpeedTestService implements ISpeedTestService {
  ...
  private setupEventHandlers(): void {
    ...
    this.eventBus.onDynamic(
      ...
      this.handleContinuousMonitoringStop.bind(this) as (
  ...
  async runComprehensiveTest(
    ...
    try {
      ...
      const savedResult = await this.speedTestRepository.create({
        ...
        status:
          pingResult.status === SpeedTestStatus.SUCCESS &&
          speedResult.status === SpeedTestStatus.SUCCESS
      ...
      this.eventBus.emitDynamic("COMPREHENSIVE_SPEED_TEST_COMPLETED", {
        targetId: config.targetId,
      ...
      this.eventBus.emitDynamic("COMPREHENSIVE_SPEED_TEST_FAILED", {
        targetId: config.targetId,
        error: error instanceof Error ? error.message : "Unknown error",
      });
  ...
  startContinuousMonitoring(
    ...
    this.eventBus.emitDynamic("CONTINUOUS_MONITORING_STARTED", {
      targetId,
      intervalMs,
    });
  }
  ...
  stopContinuousMonitoring(targetId: string): void {
    ...
    if (target) {
      ...
      this.eventBus.emitDynamic("CONTINUOUS_MONITORING_STOPPED", { targetId });
    }
  }
  ...
  pauseContinuousMonitoring(targetId: string): void {
    ...
    if (target && !target.isPaused) {
      ...
      this.eventBus.emitDynamic("CONTINUOUS_MONITORING_PAUSED", { targetId });
    }
  }
  ...
  resumeContinuousMonitoring(targetId: string): void {
    ...
    if (monitoringTarget && monitoringTarget.isPaused) {
      ...
      this.eventBus.emitDynamic("CONTINUOUS_MONITORING_RESUMED", { targetId });
    }
  }
  ...
  async runBatchTests(
    ...
    this.eventBus.emitDynamic("BATCH_SPEED_TESTS_COMPLETED", {
      targetIds,
      results,
    });
  ...
  async start(): Promise<void> {
    ...
    this.eventBus.emitDynamic("SPEED_TEST_SERVICE_STARTED", {});
  }
  ...
  async stop(): Promise<void> {
    ...
    this.eventBus.emitDynamic("SPEED_TEST_SERVICE_STOPPED", {});
  }
  ...
  private async attemptDownload(
    ...
    try {
      ...
      this.logger.debug("SpeedTestService: Attempting download", {
        url: urlConfig.url,
        name: urlConfig.name,
        expectedSize: urlConfig.sizeBytes
          ? `${urlConfig.sizeBytes / (1024 * 1024)} MB`
          : "unknown",
      });
      ...
      this.logger.debug("SpeedTestService: Download completed", {
        ...
        sizeMatch:
          urlConfig.sizeBytes && sizeInBytes === urlConfig.sizeBytes
            ? "exact"
            : "approximate",
      });
  ...
  private async measureUploadSpeed(
    ...
    try {
      ...
      try {
        ...
        if (pingResult.ping && pingResult.status === SpeedTestStatus.SUCCESS) {
  ...
  private handleContinuousMonitoringStop(data: TargetIdData): void {
