...
export class PushSubscriptionRepository implements IPushSubscriptionRepository {
  ...
  async findByUserId(userId: string): Promise<PushSubscription[]> {
    this.logger.debug(
      "PushSubscriptionRepository: Finding subscriptions by owner ID",
      { ownerId: userId }
    );
  ...
  async findByUserIdWithPagination(
    ...
    this.logger.debug(
      "PushSubscriptionRepository: Finding subscriptions by owner ID with pagination",
      { ownerId: userId, limit, offset }
    );
  ...
  async countByUserId(userId: string): Promise<number> {
    this.logger.debug(
      "PushSubscriptionRepository: Counting subscriptions by owner ID",
      { ownerId: userId }
    );
  ...
  async create(data: CreatePushSubscriptionData): Promise<PushSubscription> {
    ...
    // Convert userId to ownerId for Prisma
    const prismaData = {
      ...data,
      ownerId: data.userId,
      // Remove userId as it's not part of the Prisma model
      userId: undefined,
    };
  ...
  async deleteByUserId(userId: string): Promise<void> {
    this.logger.debug(
      "PushSubscriptionRepository: Deleting subscriptions by owner ID",
      { ownerId: userId }
    );
  ...
  private mapToPushSubscription(prismaSubscription: unknown): PushSubscription {
    const subscription = prismaSubscription as {
      ...
      p256dh: string;
      auth: string;
      ownerId: string; // Updated to use ownerId field
      createdAt: Date;
      updatedAt: Date;
    };
    ...
    return {
      ...
      userId: subscription.ownerId, // Map ownerId back to userId for the domain model
