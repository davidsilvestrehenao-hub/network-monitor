  ...
  implements ISpeedTestResultRepository
{
  ...
  private seedResults(): void {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      this.results.push({
        id: `result-${i + 1}`, // Use string ID for consistency
  ...
  async create(data: CreateSpeedTestResultData): Promise<SpeedTestResult> {
    ...
    const result: SpeedTestResult = {
      id: `result-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, // Use string ID for consistency
      ping: data.ping ?? undefined,
