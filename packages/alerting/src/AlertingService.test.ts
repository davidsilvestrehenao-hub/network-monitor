  ...
  describe("Alert Rule Management", () => {
    describe("createAlertRule", () => {
      it("should create alert rule and emit event", async () => {
        ...
        expect(result).toEqual(createdRule);
        expect(mockEventBus.emit).toHaveBeenCalledWith("ALERT_RULE_CREATED", {
          id: createdRule.id,
    ...
    describe("updateAlertRule", () => {
      it("should update alert rule and emit event", async () => {
        ...
        expect(result).toEqual(updatedRule);
        expect(mockEventBus.emit).toHaveBeenCalledWith("ALERT_RULE_UPDATED", {
          id: updatedRule.id,
          rule: updatedRule,
        });
      });
    ...
    describe("deleteAlertRule", () => {
      it("should delete alert rule and emit event", async () => {
        ...
        expect(mockAlertRuleRepository.delete).toHaveBeenCalledWith(1);
        expect(mockEventBus.emit).toHaveBeenCalledWith("ALERT_RULE_DELETED", {
          id: 1,
        });
      });
    });
  ...
  describe("Incident Management", () => {
    ...
    describe("resolveIncident", () => {
      it("should resolve an incident and emit event", async () => {
        ...
        expect(mockEventBus.emit).toHaveBeenCalledWith("INCIDENT_RESOLVED", {
          id: 1,
        });
      });
